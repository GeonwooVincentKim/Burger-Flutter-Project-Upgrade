...
class _OrdersState extends State<Orders> {
  ...
  Widget _buildOrdersAppBar() {
    return AppBar(title: Text("Orders"), centerTitle: true, actions: [
      IconButton(
        icon: Icon(IconMoon.ifilter, color: Theme.of(context).cardColor),
        onPressed: () =>
            Navigator.pushNamed(context, "/filter", arguments: "orders"),
      )
    ]);
  }
  ...
  Widget _buildOrdersOrderList(BuildContext context) {
    ...
    return Container(
        ...
        child: SingleChildScrollView(
            child: Consumer<ProviderMenu>(builder: (ctx, orderMenu, child) {
          final Map<String, dynamic> orderFilter =
              Provider.of<FilterProvider>(context, listen: false).orderFilters;
          final List<MenuModel> subMenuList = orderMenu.cartList
              .where((ordered) => checkFilter(ordered, orderFilter))
              .toList();
          // final List<MenuModel> subMenuList = orderMenu.cartList;
  ...
  Widget _buildOrdersBody(BuildContext context) {
    ...
    return Container(
      height: screenHeight,
      padding: EdgeInsets.symmetric(
          vertical: basicPadding, horizontal: basicPadding),
      child: SingleChildScrollView(
