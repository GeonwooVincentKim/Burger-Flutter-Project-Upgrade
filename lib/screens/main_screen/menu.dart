...
class _MenuState extends State<Menu> {
  ...
  void initState() {
    // categoryTypes =
    categoryTypes =
        Provider.of<ProviderMenu>(context, listen: false).menuDetail;
    if (categoryTypes == null) {
      final List<MenuModel> menuTitle =
          Provider.of<ProviderMenu>(context, listen: false).menuList;
  ...
  Widget _buildMenuBody() {
    return Container(
        padding: EdgeInsets.symmetric(
            vertical: basicPadding, horizontal: basicPadding / 2),
        margin: EdgeInsets.all(basicMargin),
        child: Consumer<ProviderMenu>(builder: (ctx, menu, child) {
          final Map<String, dynamic> menuFilter =
              Provider.of<FilterProvider>(context, listen: false).orderFilters;
          // final List<MenuModel> listMenu = menu.menuList.where((menu) => checkFilter(menu, menuFilter)).toList();
          // final MenuModel filter = Provider.of<FilterProvider>(context, listen: false).changeOrderNoMapFilters(filter);
          MenuModel categoryMenu =
          ...
          return listMenu.length == 0 || listMenu.length == null
              ...
              : GridView.builder(
                  ...
                  itemBuilder: (context, index) =>
                      ListTileMenu(menuContents: listMenu[index]),
                );
        }));
  }
